$(document).ready(function () {
    ...
    connection.on("newMessage", function (messageView) {
        ...
        var message = new ChatMessage(messageView.content, messageView.timestamp, messageView.from, isMine, messageView.avatarSrc, messageView.avatar);
        viewModel.chatMessages.push(message);
        $(".chat-body").animate({ scrollTop: $(".chat-body")[0].scrollHeight }, 1000);
    });

    connection.on("getProfileInfo", function (displayName, avatar, roleSrc) {
        viewModel.myName(displayName);
        viewModel.myAvatarSrc(roleSrc);
        viewModel.myAvatar(avatar);
    });

    connection.on("addUser", function (user) {
        viewModel.userAdded(new ChatUser(user.username, user.fullName, user.avatar, user.avatarSrc, user.currentRoom, user.device));
    });
    ...
    function AppViewModel() {
        ...
        self.myAvatarSrc = ko.observable("staff");
        ...
        self.sendNewMessage = function () {
            ...
            if (text.startsWith("/")) {
                var receiverId = 1;
                var receiverRole = "admin";
                var message = text.substring(text.indexOf(")") + 1, text.length);

                if (receiverId != 0 && receiverRole != null && message.length > 0)
                    connection.invoke("SendPrivate", receiverId, receiverRole, message.trim());
            }
            else {
    ...
    function ChatUser(userName, displayName, avatar, avatarSrc,currentRoom, device) {
        var self = this;
        self.userName = ko.observable(userName);
        self.displayName = ko.observable(displayName);
        self.avatar = ko.observable(avatar);
        self.avatarSrc = ko.observable(avatarSrc);
        self.currentRoom = ko.observable(currentRoom);
        self.device = ko.observable(device);
    }

    function ChatMessage(content, timestamp, from, isMine, avatarSrc, avatar) {
        var self = this;
